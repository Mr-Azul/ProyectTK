...
#from tkinter as tk
from tkinter import *
from Models.clientesDB import *
from reportlab.pdfgen import canvas
from View.clientesPDF import *
...
class Clientes:
	...
	def createFrames (self):
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Nombre:      ', font='10').grid(row=0, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Teléfono:    ', font='10').grid(row=1, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Correo:      ', font='10').grid(row=2, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Dirección:   ', font='10').grid(row=3, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Referencia:  ', font='10').grid(row=4, column=0)
	...
	def createButtons(self):
		self.btn_register = Button(self.framel, text='Registrar Cliente', fg='white', borderwidth=2, relief='flat', cursor='hand1', overrelief='raise', bg='#0051C8', command = lambda: self.registerProd()).grid(row=5, columnspan=2, sticky=W+E)
	...
	def createList(self):
		...
		self.list_elements.heading('col1', text='Nombre')
		self.list_elements.heading('col2', text='Teléfono')
		self.list_elements.heading('col3', text='Dirección')
		self.list_elements.heading('col4', text='Correo')
		self.list_elements.heading('col5', text='Referencia')
		self.list_elements.column('#0', anchor=CENTER, width=60)
		self.list_elements.column('col1', anchor=CENTER, width=220)
		self.list_elements.column('col2', anchor=CENTER, width=150)
		self.list_elements.column('col3', anchor=CENTER, width=230)
		self.list_elements.column('col4', anchor=CENTER, width=200)
		self.list_elements.column('col5', anchor=CENTER, width=150)
	...
	def getRow(self, event):
		...
		#Codigo
		Label(self.pop, text='Nombre: ', font=('Arial', 14)).grid(row=0, column=0)
		Entry(self.pop, textvariable=self.co, font=('Arial', 13), relief='flat').grid(row=0, column=1)
		#Nombre 
		Label(self.pop, text='Teléfono: ', font=('Arial', 14)).grid(row=1, column=0)
		Entry(self.pop, textvariable=self.na,font=('Arial', 13)).grid(row=1, column=1)
		#Proveedor 
		Label(self.pop, text='Correo: ', font=('Arial', 14)).grid(row=3, column=0)
		Entry(self.pop, textvariable=self.sup, font=('Arial', 13)).grid(row=3, column=1)
		#Precio 
		Label(self.pop, text='Dirección: ', font=('Arial', 14)).grid(row=5, column=0)
		Entry(self.pop, textvariable=self.pre, font=('Arial', 13)).grid(row=5, column=1)
		#Stock 
		Label(self.pop, text='Referencia: ', font=('Arial', 14)).grid(row=7, column=0)
	...
	def deleteRow(self, n):
		d = DataCliente()
		r = messagebox.askquestion(title='Atención', message='Desea eliminar el cliente seleccionado')
		if r == messagebox.YES:
			d.delete(n)
	...
	def registerProd(self):
		...
		if self.code.get()!='' and self.name.get()!='' and self.supp.get()!='' and self.price.get()!='' and self.cant.get()!='':
			...
			self.createList()
			self.emptyEntry()
			messagebox.showinfo(title='Atencion', message='Nuevo Cliente Registrado')
		else:
