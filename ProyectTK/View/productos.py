...
#from tkinter as tk
from tkinter import *
from Models.productosDB import *
from reportlab.pdfgen import canvas
from View.productosPDF import *
...
class Productos:
	...
	def createFrames (self):
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Codigo:      ', font='10').grid(row=0, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Nombre:      ', font='10').grid(row=1, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Proveedor:   ', font='10').grid(row=2, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Precio:      ', font='10').grid(row=3, column=0)
		self.framec = Label(self.framel, fg='white', bg='#314252', text='Stock:       ' , font='10').grid(row=4, column=0)
	...
	def createButtons(self):
		self.btn_register = Button(self.framel, text='Registrar producto', fg='white', borderwidth=2, relief='flat', 
		cursor='hand1', overrelief='raise', bg='#0051C8', command = lambda: self.registerProd()).grid(row=5, columnspan=2, sticky=W+E)
		self.btn_cancel = Button(self.framel, text='Cancelar Registro', fg='white', borderwidth=2, relief='flat', 
		cursor='hand1', overrelief='raise', bg='#E81123', command = lambda: self.detainProcess()).grid(row=6, columnspan=2, sticky=W+E)
		self.btn_pdf = Button(self.framel, text='PDF', fg='white', borderwidth=2, relief='flat', cursor='hand1', 
		overrelief='raise', bg='purple', command = lambda: self.drawPDF()).grid(row=7, columnspan=2, sticky=W+E)
		self.btn_close = Button(self.wind, text='Salir', fg='white', borderwidth=2, relief='flat', cursor='hand1', 
		overrelief='raise', bg='black', command = lambda:self.closeWind()).place(x=900, y=240, width=100)
	...
	def createList(self):
		...
		self.list_elements.heading('col1', text='Codigo')
		self.list_elements.heading('col2', text='Nombre')
		self.list_elements.heading('col3', text='Proveedor')
		self.list_elements.heading('col4', text='Precio')
		self.list_elements.heading('col5', text='Stock')
		self.list_elements.column('#0', anchor=CENTER, width=60)
		self.list_elements.column('col1', anchor=CENTER, width=150)
		self.list_elements.column('col2', anchor=CENTER, width=300)
		self.list_elements.column('col3', anchor=CENTER, width=300)
		self.list_elements.column('col4', anchor=CENTER, width=120)
		self.list_elements.column('col5', anchor=CENTER, width=80)
	...
	def getRow(self, event):
		...
		#Codigo
		Label(self.pop, text='Codigo: ', font=('Arial', 14)).grid(row=0, column=0)
		Entry(self.pop, textvariable=self.co, font=('Arial', 13), relief='flat').grid(row=0, column=1)
		#Nombre 
		Label(self.pop, text='Nombre: ', font=('Arial', 14)).grid(row=1, column=0)
		Entry(self.pop, textvariable=self.na,font=('Arial', 13)).grid(row=1, column=1)
		#Proveedor 
		Label(self.pop, text='Proveedor: ', font=('Arial', 14)).grid(row=3, column=0)
		Entry(self.pop, textvariable=self.sup, font=('Arial', 13)).grid(row=3, column=1)
		#Precio 
		Label(self.pop, text='Precio: ', font=('Arial', 14)).grid(row=5, column=0)
		Entry(self.pop, textvariable=self.pre, font=('Arial', 13)).grid(row=5, column=1)
		#Stock 
		Label(self.pop, text='Stock: ', font=('Arial', 14)).grid(row=7, column=0)
		...
		bnt_delete= Button(self.pop, text='Eliminar', relief='flat', background='red', foreground='white', 
		command = lambda: self.deleteRow(n)).grid(row=9, column=1, sticky=W+E)
	...
	def registerProd(self):
		...
		if self.code.get()!='' and self.name.get()!='' and self.supp.get()!='' and self.price.get()!='' and self.cant.get()!='':
			...
			self.createList()
			self.emptyEntry()
			messagebox.showinfo(title='Atencion', message='Nuevo Producto Registrado')
		else:
